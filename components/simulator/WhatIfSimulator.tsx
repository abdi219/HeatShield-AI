...
export function WhatIfSimulator() {
  ...
  // Executive Urban Resilience Summary Report Generator (Task 5.4)
  const handleExportReport = () => {
    if (!simulationResult) return;

    const report = `# HEATSHIELD AI — URBAN RESILIENCE MITIGATION REPORT
Date: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}
City: ${selectedCity}
Target Sector: ${simulationResult.locationName}

--------------------------------------------------
1. EXECUTIVE MICROCLIMATE SUMMARY:
- Baseline Surface Temp:     ${formatTemp(simulationResult.baselineSurfaceTemp)}${unitSymbol}
- Simulated Surface Temp:    ${formatTemp(simulationResult.simulatedSurfaceTemp)}${unitSymbol}
- Net Temperature Drop:      -${formatDelta(simulationResult.temperatureReductionDelta)}${unitSymbol} Cooler
- Heat Risk Score (HRS):     ${simulationResult.baselineHeatRiskScore}/100 -> ${simulationResult.simulatedHeatRiskScore}/100 (-${simulationResult.heatRiskScoreReductionDelta} pts)
- Estimated Cooling Radius:  ~${simulationResult.estimatedCoolingRadiusMeters} meters (~${Math.round(simulationResult.estimatedCoolingRadiusMeters / 100)} city blocks)

--------------------------------------------------
2. PARAMETRIC INTERVENTIONS DEPLOYED:
- Urban Tree Canopy Cover:   ${simulationInterventions.treeCanopyCoveragePct}%
- Cool Pavement Albedo:      ${simulationInterventions.coolPavementAlbedo.toFixed(2)} α (Solar Reflective)
- Photovoltaic Solar Canopy: ${simulationInterventions.solarCanopyCoveragePct}%
- Tensile Shade Sails:       ${simulationInterventions.shadeStructureDensityPct}%

--------------------------------------------------
3. SCIENTIFIC & ENGINEERING METHODOLOGY:
Estimates calculated using empirical urban heat island microclimate models (evapotranspiration flux, high-albedo solar reflectance, and radiant canopy interception).
--------------------------------------------------
Generated by HeatShield AI (Powered by FortyGuard Microclimate Telemetry)`;
  ...
  return (
    <div
      className={`w-80 md:w-[350px] p-4 rounded-2xl space-y-3.5 shadow-2xl overflow-y-auto max-h-[calc(100vh-100px)] scrollbar-thin ${glassCard}`}
    >
      {/* ── 1. Header ── */}
