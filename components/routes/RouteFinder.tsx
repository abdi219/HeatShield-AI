    ...
    >
      ...
      <div className={`flex items-center justify-between pb-2.5 border-b ${border}`}>
        <div className="flex items-center gap-2">
          <div className={`w-7 h-7 rounded-lg flex items-center justify-center border transition-all ${isSatellite ? "bg-white/15 backdrop-blur-md border-white/35 text-white shadow-sm" : "bg-slate-100 border-slate-200 text-slate-900 shadow-sm"
            }`}>
          ...
          {coolRoute && fastestRoute ? (
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                type="button"
                onClick={() => setIsMobileMinimized(false)}
                className={`px-2.5 py-1 rounded-lg text-[11px] font-bold shrink-0 transition-all ${isSatellite ? "bg-white text-slate-950 font-black shadow-sm" : "bg-slate-900 text-white font-bold shadow-sm"
                  }`}
              >
                Set Route ▲
              </button>
          ...
          <div className="relative">
            ...
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
              ...
              <button
                ...
                className={`p-1.5 rounded-lg text-xs transition-all ${isSatellite ? "text-white/80 hover:text-white hover:bg-white/20" : "text-slate-400 hover:text-slate-800 hover:bg-slate-200"
                  }`}
        ...
        {coolRoute && fastestRoute && (
          <div className={`space-y-3 pt-2.5 border-t ${border}`}>
            ...
            {coolRoute.exposureReductionPct > 0 && (fastestRoute.averageTempCelsius - coolRoute.averageTempCelsius) >= 0.5 ? (
              <div className={`p-2.5 rounded-xl border flex items-center justify-between ${isSatellite ? "bg-emerald-500/25 border-emerald-400/50 text-white" : "bg-emerald-50 border-emerald-200 text-emerald-950"
                }`}>
                <div className="flex items-center gap-2">
              ...
              <div className={`p-2.5 rounded-xl border flex items-center justify-between ${isSatellite ? "bg-white/10 border-white/20 text-white" : "bg-slate-100 border-slate-200 text-slate-800"
                }`}>
                <div className="flex items-center gap-2">
                ...
                {isDirectionsExpanded && (
                  <div className={`space-y-1.5 pt-2 border-t ${border} max-h-48 overflow-y-auto scrollbar-thin`}>
                    {activeRoute.steps.map((step, idx) => (
                      <div
                        key={idx}
                        className={`text-[11px] p-2 rounded-lg border space-y-1 ${isSatellite ? "bg-white/10 border-white/20" : "bg-white border-slate-200"
                          }`}
                      >
